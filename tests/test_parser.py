...
class TestExtractText:
    ...
    def test_extract_text_returns_string(self):
        ...
        with patch("pyxtxt.xtxt", return_value="sample extracted text"):
    ...
    def test_extract_text_error_message_contains_original_error(self):
        ...
        with patch("pyxtxt.xtxt", side_effect=Exception("permission denied")):
            with pytest.raises(RuntimeError) as exc_info:
                extract_text("locked.pdf")
        assert "permission denied" in str(exc_info.value)

    def test_extract_text_called_with_correct_argument(self):
        """extract_text should pass the document path to xtxt."""
        with patch("pyxtxt.xtxt", return_value="text") as mock_xtxt:
...
class TestChunkText:
    ...
    def test_chunk_text_raises_runtime_error_on_failure(self):
        """chunk_text should raise RuntimeError if the splitter fails."""
        with patch(
